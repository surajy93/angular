...
describe('NgClass migration', () => {
  ...
  it('should preserve the shared NgClass import when one of several components in the same file is only partially migrated', async () => {
    ...
    // migration, so a plain `content.includes(...)` check on the whole file can't distinguish between them.
  ...
  describe('Cases where no changes should be applied', () => {
  ...
  describe('Basic ngClass object binding migrations', () => {
    ...
    it('should migrate an object with multiple keys to individual class bindings', async () => {
      writeFile(
        '/app.component.ts',
        `
        import {Component} from '@angular/core';
  ...
  describe('Complex migrations involving multiple elements and mixed keys', () => {
  ...
  describe('Edge cases and scenarios that should not be migrated', () => {
